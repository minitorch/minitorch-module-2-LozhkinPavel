    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        return f.mul_zip(
            ...
            f.mul_zip(
                f.sigmoid_map(t1),
                f.add_zip(
                    grad_output._ensure_tensor(1.0), f.neg_map(f.sigmoid_map(t1))
                ),
            ),
        )
...
class Permute(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        inverse_order = [0 for _ in range(order.size)]
        for i in range(order.size):
            inverse_order[int(order[i])] = i
        return minitorch.Tensor(
            grad_output._tensor.permute(*(inverse_order)), backend=grad_output.backend
        ), grad_output.zeros(order.shape)
